...
<body>
    <main id="main">
        <section class="flex-center">
            <div class="img-rounded border p-1 text-center">
                <form method="post" action="{{ url()->full() }}" accept-charset="utf-8">
                    @csrf

                    <div>
                        <a href="{{ url('/') }}" class="logo mx-auto">
                            <img alt="zhivafit.com" src="{{ asset('public/assets/images/logo150.png') }}">
                        </a>
                    </div>

                    <h1 class="h3">ثبت نام</h1>
                    <p class="mb-1">«رمز عبور» دلخواه خود و «کد تایید» پیامک شده را وارد کنید</p>
                    @if($errors->any())
                        @foreach($errors->all() as $error)
                        <p class="text-danger mb-1">{{ $error }}</p>
                        @endforeach
                    @endif
                    <input class="w-100 mb-1" name="psw" type="password" placeholder="رمز عبور">
                    <input class="w-100 mb-1" name="key" type="number" placeholder="کد پیامک شده">
                    <input type="submit" class="img-rounded w-100 mb-1 bt-green" value="ثبت نام">
                    <p>ثبت نام در این سامانه به منزله پذیرفتن <a href="" title="قوانین">قوانین و مقررات</a> میباشد.</p>
