...
class userController
{
     ...
     * @return 
     */
    public function mobileToken($public_key) {
        $mobile = DB::select("SELECT `users`.`mobile` 
                    FROM (SELECT * FROM `activations` WHERE `public_key`='$public_key') `activations`
                    INNER JOIN `users`
                    ON `users`.`id`=`activations`.`userid`");
