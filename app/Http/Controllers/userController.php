...
class userController
{
	...
	public function mobileChecker() {
		...
		if (sizeof($user) == 0) {
			...
			// send sms
			$sms = app('App\Http\Controllers\api\smsController');
			$sms->send($mobile, $private, 29661);
	...
	public function registerShow() {
		...
		$mobile = $users->mobileToken($token);
		...
		$now = strtotime($now);
		$key_generate = strtotime($key[0]->key_create_at);
		...
		if ($now - $key_generate > 60*2) {
			...
			$data = array(
				'key_create_at' 		=> date('Y-m-d H:i:s'),
				'private_key' 			=> $private,
				'public_key' 			=> $token,
			);
			$mobile = $users->addKey($data);

			// send sms
			$sms = app('App\Http\Controllers\api\smsController');
			$sms->send($mobile, $private, 29660);

		}
		
		return view('login.register')->with('countdown', $now - $key_generate);
	}
	...
	public function forgetShow() {
		...
		if ($now - $key_generate > 60*2) {
			...
			// send sms
			$sms = app('App\Http\Controllers\api\smsController');
			$sms->send($mobile, $private, 29662);

		}
		
		return view('login.forget')->with('countdown', $now - $key_generate);
	}
